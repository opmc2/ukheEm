...
binnedDataExample[, right := fcase(
  ...
  binnedData == "500&over /26000&over", 600
)]
...
We now have the relevant information stored in `keyInfo`. We now want to extract the upper and lower bound to separate variables. We will use 

```{r }
lower <- c(
  1, stringr::str_extract_all(keyInfo, "[0-9]{2,}", simplify = TRUE)[2:13, 1]
)
upper <- c(
  50, 
  stringr::str_extract_all(keyInfo, "[0-9]{2,}", simplify = TRUE)[2:10, 2],
  3000
)
```
