...
resBcs <- resBcsCN_md_maleV2
maxK1 <- 4 # cognitive score
maxK2 <- 4 # noncognitive score

maxK <- length(resBcs)

# directories
figDir <- "vignettes/fig/BcsCN_md_maleV2"
...
bigDt <- merge(
  bigDt,
  bigDt[d == FALSE, .(muDFk1 = Hmisc::wtd.mean(w, pk)), by = .(nTypes, type1)],
  by = c("nTypes", "type1")
) %>% 
  merge(
    bigDt[d == FALSE, .(muDFk2 = Hmisc::wtd.mean(w, pk)), by = .(nTypes, type2)],
    by = c("nTypes", "type2")
  )
bigDt[, type1 := frank(muDFk1, ties.method = "dense"), by = .(nTypes)]
bigDt[, type2 := frank(muDFk2, ties.method = "dense"), by = .(nTypes)]
...
dtPk <- bigDt[, .(
  P_k = sum(pk) / NN,
  alpha1 = mean(alpha1),
  alpha2 = Hmisc::wtd.mean(y2, pk), alpha2b = mean(alpha2),
  muCons = Hmisc::wtd.mean(w, pk)
), by = .(nTypes, nTypes1, nTypes2, type, type1, type2)]

dtPk[, alpha1Norm := (alpha1 - mean(alpha1)) / sd(alpha1), by = nTypes]
dtPk[, alpha2Norm := (alpha2 - mean(alpha2)) / sd(alpha2), by = nTypes]
dtPk[, muConsNorm := (muCons - mean(muCons)) / sd(muCons), by = nTypes]
...
dtPk[nTypes == "32" & variable == "alpha1Norm", .(type, P_k, alpha1, alpha2b, muCons)]

for (k1 in 1:3) {
  # for (k2 in 1:2) {
    ggplot(
  data = dtPk[nTypes == "32" & type1 == k1],
  mapping = aes(x = type)
) +
  geom_col(aes(y = P_k)) +
  geom_point(aes(y = (value + 4)/6, colour = variable)) +
  ...
  theme_classic() +
  theme(legend.position = "bottom", text = element_text(size = 15))
  saveFig(paste0("groupSizesMeansK32_k1", k1, ".png"))
  # }
}
...
# for (k1 in 1:3) {
  ...
  scale_color_manual(name = NULL, 
                     limits = c("alpha1Norm", "alpha2Norm", "muConsNorm"),
                     labels = c("Cognitive", "Noncognitive", "Wage @25"),
                     values = c("red", "blue", "green")) +
  theme_classic() +
  theme(legend.position = "bottom", text = element_text(size = 15))
  saveFig(paste0("groupSizesMeansK32_k2", k2, ".png"))
  }
# }
```
...
ggplot(data = dtWPuncond,
  ...
  scale_colour_manual(name = NULL,
                      ...
                      limits = c("ATE", "ATT")) +
  theme_classic() +
  theme(text = element_text(size = 25), legend.position = c(.9, .15))
