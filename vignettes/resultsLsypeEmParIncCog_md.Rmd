...
# directories
figDir <- "vignettes/fig/LsypeParIncCog_md"
dir.create(here(figDir))

resLsype <- resLsypeParIncCog_md

maxK1 <- 6 # parental income
maxK2 <- 6 # cognitive measure
maxK <- 24
```
...
for (K1 in 2:maxK1) {
  for (K2 in 2:maxK2) {
    bigDt[[i]] <- resLsype[[i]][[3]]
    bigDt[[i]][, c("nTypes1", "nTypes2", "nTypes") := .(K1, K2, paste0(K1, K2))]
    if (i == maxK) break
    i <- i+1
  }
}
...
for (K1 in 2:maxK1) {
  for (K2 in 2:maxK2) {
    dtLikeConverge[[i]] <- data.table(
      ...
      iter = 1:length(resLsype[[i]]$listLoglike),
      logLikelihood = unlist(resLsype[[i]]$listLoglike)
    )
    if (i == maxK) break
    i <- i+1
  }
...
), by = .(nTypes, nTypes1, nTypes2, type)]
...
ggplot(
  ...
  facet_grid(row = vars(nTypes1), cols = vars(nTypes2), 
             labeller = label_both) +
...
), by = .(nTypes, nTypes1, nTypes2, type, d)]

dtWagePrem <- dcast(dtWagePrem, 
                    formula = nTypes + nTypes1 + nTypes2 + type ~ d, 
...
dtWagePrem <- merge(dtWagePrem,
                    bigDt[, .(P_k = sum(pk) / NN), by = .(nTypes, types)],
                    by = c("nTypes", "type"))
...
ggplot(data = dtWagePrem,
  ...
  facet_grid(rows = vars(nTypes1), cols = vars(nTypes2), scales = "free") +
  scale_x_discrete(name = "Type (k)") +
...
dtWPuncond[, nTypes := factor(nTypes, nTypesLevels)]
