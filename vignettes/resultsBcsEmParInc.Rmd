...
# functions
signiff <- function(x, ...) if (is.numeric(x)) signif(x, digits = 4)
...
# directories
figDir <- "vignettes/fig/BcsParInc"
...
ggplot(
  ...
  scale_x_continuous(name = "Number of types (G)", breaks = seq(2, 18, by = 2)) +
...
ggplot(
  ...
  scale_color_discrete(name = NULL, 
                       ...
                       labels = c("Parental Inc. @16", "Wage @25")) +
  theme_classic() +
...
ggplot(data = dtWagePrem[nTypes %between% c(2, 9)],
       mapping = aes(x = type, y = wagePrem, alpha = P_k)) +
  geom_col(position = "dodge", fill = "red") +
  facet_wrap(vars(nTypes), nrow = 2) +
  scale_x_discrete(name = "Type (k)") +
  scale_y_continuous(name = "Wage prem. (% change)") +
...
dtPkd <- bigDt[, .(
  ...
  mu = mean(mu), sigmaNu = mean(sigmaNu),
  alpha = Hmisc::wtd.mean(y2, pk),
...
ggplot(
  ...
  scale_x_discrete(name = "Type (k)") +
  scale_fill_discrete(name = NULL, limits = c(FALSE, TRUE), 
                      labels = c("No degree", "First or higher degree")) +
  theme_classic() +
...
ggplot(
  data = dtPkd[nTypes %between% c(10, 15), ],
  mapping = aes(x = type, fill = d)
) +
  geom_col(aes(y = P_kd)) +
  geom_point(aes(y = (muNorm + 4)/6, shape = "Parental Inc. @16"), stroke = .3) +
...
dtAssignedTypes <- merge(
  ...
  merge(dtBcs4Em[, .(id = bcsid, locScore, selfEsteemScore, ghqScore)],
        by = "id")
...
dtAssignedTypes[nTypes == 2, .(prop = .N / NN), by = rgsc91][order(rgsc91)]
```
...
dtSumStats_HiLoTypes <- dtAssignedTypes[nTypes %between% c(3,5) & !is.na(group), .(
  # "Par. Inc. @16 (mean)" = mean(exp(mu)),
  # "Par. Inc. (std dev.)" = exp(mean(sigmaNu)),
  "Wage (mean)" = mean(exp(y2)),
  "Wage (std dev.)" = sd(exp(y2)),
...
xtable::xtable(dtSumStats_HiLoTypes, digits = 2)

dtAssignedTypes[!is.na(rgsc91) & !is.na(group) & nTypes %between% c(3, 5), grpSize := .N, by = .(group, d)]

dtAssignedTypes[nTypes %between% c(3, 5), .(prop = .N / mean(grpSize)), 
                by = .(group, rgsc91, d)][order(group, d, rgsc91)]
```
...
dtAssignedTypes[!is.na(group) & nTypes %between% c(3, 5) & !is.na(NSSEC5), 
                .(prop = .N / unique(grpSize)), by = .(group, NSSEC5, d)][order(NSSEC5, group, d)]
```
