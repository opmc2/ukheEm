...
dtMu <- bigDt[, .(
  # P_k = sum(pk) / NN,
  alpha1 = Hmisc::wtd.mean(left+right, pk),
  alpha1_mean = mean(mu),
...
dtAssignedTypes <- merge(
  ...
  merge(dtBcs4Em[, .(id = bcsid, combnScore, locScore, selfEsteemScore, ghqScore, left, right)],
        by = "id")
...
dtSumStatsK3 <- dtAssignedTypes[nTypes == 3, .(
  ...
  "Cognitive ability" = mean(combnScore, na.rm = TRUE),
...
xtable::xtable(dtSumStatsK3, digits = 0)
...
dtAssignedTypes[nTypes == 3 & type == 1 & d == F, .(prop = .N / 704), by = .(soc4)]
dtAssignedTypes[nTypes == 3 & type == 1 & d == T, .(prop = .N / 92), by = .(soc4)]
dtAssignedTypes[nTypes == 3 & type == 2 & d == F, .(prop = .N / 626), by = .(soc4)]
dtAssignedTypes[nTypes == 3 & type == 2 & d == T, .(prop = .N / 324), by = .(soc4)]
dtAssignedTypes[nTypes == 3 & type == 3 & d == F, .(prop = .N / 55), by = .(soc4)]
dtAssignedTypes[nTypes == 3 & type == 3 & d == T, .(prop = .N / 158), by = .(soc4)]
```
