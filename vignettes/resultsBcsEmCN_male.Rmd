...
dtPkByD <- bigDt[, .(
  ...
  sigmaD = sqrt(Hmisc::wtd.var(w, pk))
), by = .(nTypes, type, d)]
...
dtSigmaW <- bigDt[, .(sigmaW = sqrt(sum(pk * (w - mu)^2) / sum(pk))), by = .(nTypes, d)]

dtPkzd <- bigDt[, .(
    pkzd = sum(pk) / NN
  ), by = .(nTypes, type, z, d)]

dtPkzdK5 <- dcast(dtPkzd[nTypes == 5], z ~ type + d, value.var = "pkzd")
...
dtMuByD <- bigDt[, .(
  ...
  mu_mean = mean(mu), sigmaW = mean(sigmaW),
  mu = Hmisc::wtd.mean(w, pk)
), by = .(nTypes, type, d)]

bigDt <- merge(
  bigDt, 
  dtMuByD[d == TRUE, .(nTypes, type, muD1 = mu, sigD1 = sigmaW)],
  by = c("nTypes", "type")
) %>% 
  merge(
    dtMuByD[d == FALSE, .(nTypes, type, muD0 = mu, sigD0 = sigmaW)],
    by = c("nTypes", "type")
  )
...
dtMeasures <- bigDt[nTypes == K, .(
  cog = sum(pk*alpha1), noncog = sum(pk*alphaNc), w1 = sum(pk*muD1, na.rm = T), 
  w0 = sum(pk*muD0, na.rm = TRUE), d = unique(d)
), by = id]
...
ggplot() +
  ...
  annotate(geom = "label", x = 52, y = 0.33, label = "Graduates (d = 1)", colour = "#00BFC4") +
  annotate(geom = "label", x = 58, y = -.25, label = "Non-graduates (d = 0)", colour = "#F8766D") +
  annotate(geom = "label", x = 47, y = -.22, label = "Type 1") +
  annotate(geom = "label", x = 48, y = 0.2, label = "Type 2") +
  annotate(geom = "label", x = 60, y = 0.4, label = "Type 4") +
  annotate(geom = "label", x = 71, y = 0.0, label = "Type 3") +
  annotate(geom = "label", x = 78, y = 0.4, label = "Type 5") +
...
ggplot() +
  ...
  annotate(geom = "label", size = 5, x = 52, y = 0.33, label = "Graduates (d = 1)", colour = "#00BFC4") +
  annotate(geom = "label", size = 5, x = 58, y = -.25, label = "Non-graduates (d = 0)", colour = "#F8766D") +
  annotate(geom = "label", size = 5, x = 47, y = -.22, label = "Type 1") +
  annotate(geom = "label", size = 5, x = 48, y = 0.2, label = "Type 2") +
  annotate(geom = "label", size = 5, x = 60, y = 0.4, label = "Type 4") +
  annotate(geom = "label", size = 5, x = 71, y = 0.0, label = "Type 3") +
  annotate(geom = "label", size = 5, x = 78, y = 0.4, label = "Type 5") +
  scale_size_area(max_size = 20) +
  scale_x_continuous(name = "Cognitive") +
  scale_y_continuous(name = "Noncognitive") +
  theme_bw() +
...
## Group sizes by education

```{r group sizes table}

bigDt[nTypes == 5, round(sum(pk * d) / sum(pk), digits = 2), by = type]

bigDt[nTypes == 5, wtd.sd(exp(w), pk), by = .(type, d)]
bigDt[nTypes == 5, exp(Hmisc::wtd.mean(w, pk)), by = type]

bigDt[nTypes == 5, round(sum(pk) / NN, digits = 2), by = .(type, d)]
...
ggplot(data = dtMeasuresAll[wagePrem %between% c(-1, 1)], aes(x = wagePrem, colour = d)) +
  geom_density(adjust = 1.5) +
  # annotate(geom = "label", x = .35, y = 4.3, label = "Graduates (d = 1)", colour = "#00BFC4") +
  # annotate(geom = "label", x = -0.10, y = 1.8, label = "Non-graduates (d = 0)", colour = "#F8766D") +
  facet_wrap(~nTypes, scales = "free") +
  theme_classic() +
...
```{r correlation between wagePrem and cognitive / noncognitive ability}
lmWagePrem1 <- dtMeasuresAll[nTypes == 8, lm(wagePrem ~ lCog + lNoncog)]
lmWagePrem2 <- dtMeasuresAll[nTypes == 8, lm(wagePrem ~ lCog + lNoncog + lCog:lNoncog)]
lmWagePrem3 <- dtMeasuresAll[nTypes == 8, lm(wagePrem ~ lCog + I(lCog^2) + lNoncog + I(lNoncog^2) + lCog:lNoncog)]

stargazer::stargazer(lmWagePrem1, lmWagePrem2, lmWagePrem3, type = "text")
```
...
dtWagePrem <- dcast(dtWagePrem, 
                    ...
                    value.var = c("alpha1d", "alpha2d", "mu", "sigmaW"))
...
ggplot(data = dtWagePrem[nTypes %between% c(2, 7)],
  ...
  facet_wrap(vars(nTypes), nrow = 2, scales = "free") +
  scale_x_discrete(name = "Type (k)") +
...
ggplot() +
  ...
  annotate(geom = "label", x = 46.0, y = -.24, label = "Type 1") +
  annotate(geom = "label", x = 47.5, y = 0.14, label = "Type 2") +
  annotate(geom = "label", x = 59.5, y = 0.38, label = "Type 4") +
  annotate(geom = "label", x = 70.5, y = 0.01, label = "Type 3") +
  annotate(geom = "label", x = 77.5, y = 0.42, label = "Type 5") +
...
ggplot() +
  ...
  # annotate(geom = "label", x = 65, y = 0.42, label = "Graduates (d = 1)", colour = "#00BFC4") +
  # annotate(geom = "label", x = 66, y = 0.06, label = "Non-graduates (d = 0)", colour = "#F8766D") +
  annotate(geom = "label", x = 46.0, y = -.24, label = "Type 1", size = 5) +
  annotate(geom = "label", x = 47.5, y = 0.14, label = "Type 2", size = 5) +
  annotate(geom = "label", x = 59.5, y = 0.38, label = "Type 4", size = 5) +
  annotate(geom = "label", x = 70.5, y = 0.01, label = "Type 3", size = 5) +
  annotate(geom = "label", x = 77.5, y = 0.42, label = "Type 5", size = 5) +
  scale_size_continuous(range = c(10,20)) +
  scale_x_continuous(name = "Cognitive") +
  scale_y_continuous(name = "Noncognitive") +
  theme_bw() +
...
bigDt[nTypes == K, .(Hmisc::wtd.var(w, pk), Hmisc::wtd.mean(w, pk)), by = .(type, d)][
...
### Table 7: correlations using assigned types

```{r assigned types correlations}

lmWagePrem1AT <- dtAssignedTypes[nTypes == 5, lm(muD1 - muD0 ~ alpha1 + alpha2 -1)]
lmWagePrem2AT <- dtAssignedTypes[nTypes == 5, lm(muD1 - muD0 ~ alpha1 + alpha2 + alpha1:alpha2-1)]
lmWagePrem3AT <- dtAssignedTypes[nTypes == 5, lm(muD1 - muD0 ~ alpha1 + I(alpha1^2) + alpha2 + I(alpha2^2) + alpha1:alpha2-1)]

stargazer::stargazer(lmWagePrem1AT, lmWagePrem2AT, lmWagePrem3AT, type = "text")
```
