---
title: "Descriptive statistics + probability model (LSYPE)"
...
dtLsypeWorkerFirm <- lsypeW8[, .(
  NSID, sex = W8CMSEX, ethnicGroup = W8DETHN6,
  # yrJobStart = b960270,
  inCouple = W8PART,
  married = W8CRNOWMA,
  cohabit = W8NRANY,
...
# occupations
dtLsype4SS[degree25 == "No degree", .(prop = .N / 669), by = .(NSSEC5)]
dtLsype4SS[degree25 == "First or higher degree", .(prop = .N / 978), by = .(NSSEC5)]

# ethnic group
dtLsype4SS[degree25 == "No degree", .(prop = .N / 669), by = .(ethnicGroup)]
dtLsype4SS[degree25 == "First or higher degree", .(prop = .N / 978), by = .(ethnicGroup)]

dtLsype4SS[, (prop = .N / 1647), by = ethnicGroup]
```
```{r ols wages}

dtLsype4Ols <- merge(
  dtLsype, dtLsype4Em[, .(NSID, logWkPay, logParInc, cogScore, degree25,
                          sex, att2schlScr, locScore, ghqScr)],
  by = "NSID", all.y = TRUE
) %>% 
  merge(dtLsypeWorkerFirm[, .(NSID, ethnicGroup)], by = "NSID", all.x = TRUE)

wageModelLsype <- lm(
  logWkPay ~ logParInc + cogScore + degree25 + W4BenefitsYP0b + sex + 
    att2schlScr + ethnicGroup +
    locScore + ghqScr
  ,
  data = dtLsype4Ols
)

summary(wageModelLsype)
```
