...
ggplot(
  ...
  scale_color_manual(name = NULL, 
                     limits = c("muNorm", "muBNorm", "alphaConsNorm"),
...
dtPkd <- bigDt[, .(
  P_kd = sum(pk) / NN,
  muBd = Hmisc::wtd.mean(y1b, pk),
  sigmaNuBd = sqrt(Hmisc::wtd.var(y1b, pk, normwt = TRUE)),
...
dtPkd[, muBdNorm := (muBd - mean(muBd)) / sd(muBd)]
...
ggplot(
  ...
  geom_point(aes(y = (muBdNorm + 4)/6, shape = "Test score @16"), stroke = .3) +
