...
ggplot(
  ...
  guides(shape = guide_legend()) +
  theme(text = element_text(size = 15), legend.position = "bottom") +
  coord_cartesian(ylim = c(5.8, 6.5))
...
dtAssignedTypes <- merge(
  dtAssignedTypes[, .(nTypes, id, y1, w, z, d, type, pk)], 
  dtLsypeWorkerFirm, 
  by = "id"
) %>% 
  merge(dtLsype4Em[, .(id = NSID, cogScore, locScore, att2schlScr, ghqScr, logParInc)])

```

```{r sumstats}

dtSumStatsK3 <- dtAssignedTypes[nTypes == 2, .(
  "Type" = unique(type),
...
# occupations
dtAssignedTypes[nTypes == 2, .N, by = .(type, d)][order(type,d)]
dtAssignedTypes[nTypes == 2 & type == 1 & d == "No degree", .(prop = .N / 377), by = .(NSSEC5)]
dtAssignedTypes[nTypes == 2 & type == 1 & d == "First or higher degree", .(prop = .N / 712), by = .(NSSEC5)]
dtAssignedTypes[nTypes == 2 & type == 2 & d == "No degree", .(prop = .N / 28), by = .(NSSEC5)]
dtAssignedTypes[nTypes == 2 & type == 2 & d == "First or higher degree", .(prop = .N / 108), by = .(NSSEC5)]
